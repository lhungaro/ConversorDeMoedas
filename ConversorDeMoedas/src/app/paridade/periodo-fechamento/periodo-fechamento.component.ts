...
@Component({
  selector: 'app-periodo-fechamento',
  templateUrl: './periodo-fechamento.component.html',
  styleUrls: ['./periodo-fechamento.component.css']
})
export class PeriodoFechamentoComponent implements OnInit {
  ...
  getCotacaoMoedaPeriodoFechamento(moeda:string,dataIncial : string,dataFinal : string): void {
    this.paridadeCotacao.getCotacaoMoedaPeriodoFechamento(moeda, dataIncial, dataFinal ).subscribe({
      next: (result:any) => {
  ...
  onSubmit() {
    ...
    this.getCotacaoMoedaPeriodoFechamento(this.formulario.moeda,this.formulario.dataInicial,this.formulario.dataFinal);
  }
